...
def Task4(db):
	...
	ackDict = {}
	for packet in db:
		if packet.srcIp == "130.245.145.12" and packet.destIp == "128.208.2.198" and packet.seqNum not in seqDict:
			seqDict[packet.seqNum] = packet.timestamp
		elif packet.srcIp == "128.208.2.198" and packet.destIp == "130.245.145.12" and packet.ackNum not in ackDict:
			ackDict[packet.ackNum] = packet.timestamp	

	transactions = 0
	totalTime = 0
	for sno, sts in seqDict.iteritems():
		if sno in ackDict:
			transactions += 1
			totalTime += ackDict[sno] - sts

	# print "Total RTT : " + str(totalTime) + "No of transactions : " + str(transactions)
	print "Average RTT : " + str(totalTime/transactions)
...
def Task3(db):
	...
	print "Packets not recieved : " + str(loss) + " TotalSent : " + str(totalSent)
	print "Loss Rate : " + str(loss*1.0/totalSent)
...
def main():
	...
	for conn in connections:
		#Find payload for each connection
		Task2(conn.packets)
