...
def checkIfAllNegativeWeights():
        global routingTable
        for x in routingTable.keys():
                if routingTable[x][0] > 0:
                        return False
...
def OnRecieve(data):
        ...
        if checkIfAllNegativeWeights() == True:
                lock.release()
                return
        ...
        if updated:
                ...
                # print routingTable
                printRoutingTable()
                print "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Updated Routing Table at " + str(time.time()) + " %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
...
def localfilequery():
        while 1:
                ...
                time.sleep(5)
                dist_vector = populate_dvt('table.txt')
                payload = json.dumps([dist_vector, hostname])
                # print payload
                OnRecieve(payload);



def ListenLocalRoutingThread():
        global hostname, hostip, hostport, routingTable, neighbor_dict
        t = Thread(target=localfilequery, args=())
        t.start()                        
...
if __name__ == '__main__':
        ...
        ListenLocalRoutingThread()

        # print hostip, hostport

        time.sleep(10)
        print "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% " + hostname + " is active at " + str(time.time()) + " %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
